...
export class CommodityDetailComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['commodity'] && this.historyCtx) {
      // Subsequent updates after initial view init
      this.drawAll();
    }
  }
  ngOnInit() {
    // 1) load commodity by name
    const name = this.route.snapshot.paramMap.get('name')!;
    this.sub.add(
      this.svc.getByName(name).subscribe(c => {
        if (!c) {
          // not found → back to market
    ...
    // 2) redraw when days toggle changes
    this.sub.add(
  ...
  private drawHistoryChart() {
    ...
    this.historyChart = new Chart(this.historyCtx.nativeElement.getContext('2d')!, {
      type: 'bar',
      data: <ChartData<'bar'>>{
        labels,
        datasets: [
      ...
      options: <ChartOptions>{
        ...
        scales: {
          ...
          'y-volume': { type: 'linear', position: 'right', beginAtZero: true }
        },
  ...
  private drawRegionScatter() {
    ...
    this.regionChart = new Chart(this.regionCtx.nativeElement.getContext('2d')!, {
      type: 'scatter',
